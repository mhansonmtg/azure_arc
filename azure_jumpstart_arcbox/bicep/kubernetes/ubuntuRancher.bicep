...
resource vmInstallscriptK3s 'Microsoft.Compute/virtualMachines/extensions@2022-03-01' = {
  ...
  properties: {
    ...
    settings: {}
    protectedSettings: {
      commandToExecute: 'bash installK3s.sh ${adminUsername} ${subscription().subscriptionId} ${vmName} ${azureLocation} ${stagingStorageAccountName} ${logAnalyticsWorkspaceResourceId} ${templateBaseUrl} ${storageContainerName} ${k3sControlPlane}'
      fileUris: [
